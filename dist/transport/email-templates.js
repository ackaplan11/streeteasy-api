...
function buildSubject(label, payload, prefix) {
    const parts = [];
    if (payload.newListings.length > 0) {
        parts.push(`${payload.newListings.length} new listing${payload.newListings.length === 1 ? "" : "s"}`);
    }
...
function buildHtml(label, payload) {
    const sections = [];
    sections.push(`<h2 style="font-family:system-ui,sans-serif;margin:0 0 12px">${escapeHtml(label)}</h2>`);
    if (payload.newListings.length > 0) {
    ...
    if (payload.priceDrops.length > 0) {
        sections.push(`<h3 style="font-family:system-ui,sans-serif">Price drops</h3>`);
